...
Онлайн-тренажёр для самостоятельного изучения Python и AI-агентов: практический курс на 2–3 дня с ИИ-напарником, чистая чёрно-белая тема и дополнительные цветовые пресеты, тесты с проходным баллом 70% и реальный Python прямо в браузере через Pyodide.

Актуальные первоисточники и добавленные темы зафиксированы в [sources.md](sources.md).

Обязательное ядро рассчитано на 2–3 дня по 2–3 часа и состоит из 14 шагов: Python с нуля, данные и JSON, LLM API, цикл агента, инструменты, контекст, function calling и проверка перед первым заказом. Остальные темы остаются в библиотеке для углубления.
